...
const ControlButton = ({ clickHandler, buttonCaption }: Props) => {
  return (
    <button onClick={clickHandler} className="grid z-[1] justify-items-center" style={{ cursor: (buttonCaption === "Pending" ? "not-allowed" : "pointer") }} disabled={buttonCaption === "Pending" ? true : false}>
      <ControlButtonLGBack />
      <ControlButtonMDBack />
      <div className="font-['Trouble_Font'] text-[40px] tracking-[2px] mt-[-82.5px] md:mt-[-87.5px] pointer-events-none">{buttonCaption !== "Pending" ? buttonCaption : (
        <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white w-[32px] h-[32px] ml-[6px] mt-[12px]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
